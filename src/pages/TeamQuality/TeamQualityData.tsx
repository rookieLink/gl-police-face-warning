...
export default function TeamQualityData() {
  ...
  const fetchFileList = useCallback(async (currentPage = 1, pageSize = 10) => {
    setLoading(true);
    try {
      const res = await getFileList({ currentPage, pageSize, forUse: 2 });
  ...
  const handleRename = async () => {
    ...
    try {
      await renameFile({ filename: renameFilename, new_name: newFilename.trim(), forUse: 2 });
  ...
  const handleDetail = async (filename: string) => {
    ...
    try {
      const res = await getFileInfo(filename, 2);
      setDetailData(res);
    } catch {
  ...
  const handleAnalysis = (filename: string) => {
    navigate(`/team-quality/analysis?filename=${encodeURIComponent(filename)}`);
  };
      ...
      render: (_, record) => (
        <Space size={[4, 4]} wrap>
          <Button
            ...
            onClick={() => handleAnalysis(record.filename)}
            style={{ padding: 0 }}
          >
            分析
          </Button>
  ...
  return (
    <div>
      <h2 style={{ marginBottom: 16 }}>质态数据</h2>
